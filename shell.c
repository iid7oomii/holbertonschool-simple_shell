...
/**
 * trim_spaces - trim leading and trailing spaces/tabs
 * @s: input string (modified in place)
 *
 * Return: pointer to trimmed start
 */
...
/**
 * build_argv - split command line into argv (modifies input)
 * @cmd: command line (modified)
 * @argv: array to fill
 * @size: max entries
 *
 * Return: argc
...
int fork_and_exec(char *path, char **argv, char **envp, char *pname, int line_no)
{
	...
	if (pid == 0)
	{
		...
		/* if execve fails in child, print formatted message and exit */
...
/**
 * execute_command - execute command line and return status
 * @linebuf: command line buffer
 * @pname: program name for errors
 * @envp: environment vars
...
int execute_command(char *linebuf, char *pname, char **envp, int line_no)
{
	...
	if (!path)
	{
		/* print exact formatted message required by checker */
		dprintf(STDERR_FILENO, "%s: %d: %s: not found\n", pname, line_no, argv[0]);
		free(dup);
		/* if non-interactive (pipe), exit with 127 */
		if (!isatty(STDIN_FILENO))
			exit(127);
		return (127);
	}
